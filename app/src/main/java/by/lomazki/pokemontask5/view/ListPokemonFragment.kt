...
import by.lomazki.pokemontask5.viewmodel.PokemonViewModel
import by.lomazki.pokemontask5.viewmodel.adapter.PokemonAdapter
...
class ListPokemonFragment : Fragment() {
            ...
            onPokemonClicked = {
                findNavController().navigate(
                    ListPokemonFragmentDirections
                        .actionFragmentMainToFragmentDetails(it.url,it.name)
                )
            })
    }
    private val viewModel by viewModels<PokemonViewModel> {
        viewModelFactory {
            initializer {
                PokemonViewModel(ServiceLocator.provideDataSource())
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            viewModel
                .currentLceFlow
//              отписываемся от получения данных при дестрое вьюмодели и подписаться при запуске
